...
$id = $title = $description = $url = $startsAt = $endsAt = $category = $location = $genderRepresentation = $award = $imageUrl = $winner = $runnerUp = "";
$titleError = $descriptionError = $urlError = $startsAtError = $endsAtError = $categoryError = $locationError = $genderRepresentationError = $awardError = $imageUrlError = $winnerError = $runnerUpError = "";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($imageUrl == '') {
        $imageUrlError = 'Ends at field is required!';
    }

    if ($titleError == "" && $descriptionError == "" && $urlError == "" && $startsAtError == "" && $endsAtError == "" && $imageUrlError == "" && $awardError == "") {
        require "../config.php";

        $sql = "UPDATE broadcast SET title='$title', description='$description', category_id=$category, location='$location', gender_representation='$genderRepresentation', url='$url', starts_at='$startsAt', ends_at='$endsAt', award='$award', image_url='$imageUrl', winner='$winner', runner_up='$runnerUp' WHERE id = $id";
