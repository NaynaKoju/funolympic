...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT n.id, n.title, n.description, c.name as category, n.image_url, n.created_at FROM news n INNER JOIN category c on n.category_id = c.id WHERE n.id={$newsId};";
} else {
    header("Location: /pd-nayna/news.php");
    die();
}
    ...
    <?php
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<img src="'.$row['image_url'].'" width="100%" height="400px" style="object-fit: cover;" />';
                echo '<div style="margin: 18px">';
                echo '<h1>' . $row['title'] . '</h1>';
                echo '<p style="font-weight: bold; font-size: 14px;">' . $row['category'] . '</p>';
                echo '<p>' . $newFormat . '</p>';
                echo '<p style="padding-top: 24px;">' . $row['description'] . '</p>';
                echo '</div>';
                // echo '<article>';
                // echo '<h4>just in</h4>';
                // echo '<div>';
                ...
                // echo '<a href="#">Read More <span>>></span></a>';
                // echo '</div>';
                // echo '</article>';
            }
            }
