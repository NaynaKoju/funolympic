...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($imageURL == '') {
        $imageURLError = 'Image Url field is required!';
    }

    if ($titleError == "" && $descriptionError == "" && $categoryError == "") {
        require "../config.php";

        // $formattedDesc = str_replace("'", "|='", $description);

        $sql = "INSERT INTO news (title, description, category_id, created_at, image_url) VALUES ('{$title}', '{$description}', '{$category}', NOW(), '{$imageURL}')";
